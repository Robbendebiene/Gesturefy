...
import DefaultGestures from '/resources/json/default-gestures.json' with { type: 'json' };

/**
 * Main access point to handle gestures.
 * Gestures are stored in the local storage.
 * Gestures are loaded on startup and updated on storage changes.
...
export default class GestureManager extends BaseEventListener {
  ...
  #storageChangeHandler(changes, areaName) {
    if (areaName === 'local' && changes.hasOwnProperty('Gestures')) {
      const { newValue, oldValue } = changes['Gestures'];
      this.#gestures = this.#parseGestures(newValue);
      this._dispatchEvent('change', this);
    }
  }

  #parseGestures(gestures) {
    const rawGestures = Array.isArray(gestures) ? gestures : DefaultGestures;
    return rawGestures.map(Gesture.fromJSON);
  }
  ...
  /**
   * Overwrites the current gestures with the given gestures.
   * The given gestures must be array like or iterable.
   **/
  async setAll(gestures) {
    this.#gestures = Array.from(gestures);
    await browser.storage.local.set({
      'Gestures': this.#gestures.map(g => g.toJSON())
    });
  }

  /**
   * Returns the gesture that matches the given pattern the most.
  ...
  /**
   * Cleanup manager resources and dependencies
   **/
  dispose() {
